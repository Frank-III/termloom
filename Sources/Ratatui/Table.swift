...
extension Table where Row == TableRow {
  /// Creates a table from row-owned cells, matching Ratatui's sequential cell model.
  ///
...
/// Per-row geometry and style, corresponding to Ratatui's row-owned presentation.
...
public struct Table<Row>: Widget, StatefulWidget {
  ...
  /// Table columns use Ratatui's solver boundaries, which intentionally differ
  ...
  /// Resolves equal-priority fixed widths using Ratatui's rounded solver boundaries.
