...
TermLoom Swift should be useful as a small rendering library without forcing applications to adopt chat,
...
No HTTP, transcript, provider, Markdown-stability, request-library, or chat policy belongs in `TermLoom`.

The `TermLoom` target itself has no dependencies. The root package now uses SwiftPM traits to prevent optional
products from pulling unrelated transitive dependencies:

- `SyntaxHighlighting` gates the Highlight products used by `TermLoomSyntaxHighlighting`.
...
A clean minimal consumer resolves only `termloom`; the Postcat example resolves only `swift-highlight` in
addition to the local ecosystem packages, while DiffScope uses only core and `TermLoomOverlays`. Snapshot testing,
...
- editable URL and multiline JSON body powered by `TermLoomTextArea`;
...
also consumes `TermLoomTextArea` and `TermLoomOverlays`, plus an opt-in `DevTools` trait backed by
`TermLoomDevTools`, proving the sibling packages compose in a real app without making diagnostics mandatory.
...
`TermLoomOverlays`, making it an additional dependency-boundary check.
...
- keep `TermLoom` small and behaviorally aligned with TermLoom's rendering/terminal invariants;
...
- consider `TermLoomKeymap`, `TermLoomQRCode`, and an SSH terminal host only after real clients establish APIs.
...
| `tui-textarea` / `termloom-textarea` | Initial multiline editor, selection, undo, viewport package built | `TermLoomTextArea` |
| `tui-popup` / command palettes | Popup geometry/presentation package built | `TermLoomOverlays` |
| `tui-tree-widget` | Missing hierarchical expansion and navigation | `TermLoomTree` |
| `termloom-image` | No Kitty/Sixel/iTerm2 protocol abstraction | `TermLoomImage` |
| `tui-logger` | No log store, filtering, levels, tail-follow widget | `TermLoomLogger` |
| `termloom-explorer` | Filesystem semantics correctly absent from core | `TermLoomFileExplorer` |
| `termloom-themes` | Styles exist; semantic theme catalogs do not | `TermLoomThemes` |
| `tui-big-text`, throbbers, extra charts | Some metrics exist; decorative widgets vary | `TermLoomWidgets` |
| Markdown/rendering crates | Codex currently owns Markdown semantics | `TermLoomMarkdown` only after a second client |
| Form/grid crates | Tables exist, editable forms do not | `TermLoomForms` |
| OpenTUI console / React DevTools | Initial metrics/log panel and optional overlay built | `TermLoomDevTools` |
| Declarative framework adapters | Result builders plus optional single-pass conformance synthesis | `TermLoomMacros` |

`TermLoomSyntaxHighlighting` and `TermLoomTestSupport` demonstrate conditional products within the root package.
`TermLoomTextArea`, `TermLoomOverlays`, `TermLoomDevTools`, and `TermLoomMacros` are independent sibling packages.
...
1. **Harden the new sibling packages.** Add composition examples and terminal coverage for `TermLoomTextArea`,
   `TermLoomOverlays`, `TermLoomDevTools`, and `TermLoomMacros` before expanding their APIs.
